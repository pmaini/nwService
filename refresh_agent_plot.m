...
for i = 1:numAgent
    ...
    if(agent_node(i).task_assigned > 0)
        if mod(agent_node(i).current_task,2) == 0
            agent_node(i).plot(1) = plot(agent_node(i).xc,agent_node(i).yc,'mo','MarkerFaceColor','magenta','MarkerSize',10);
        else
    ...
    elseif (agent_node(i).in_relay == 1)
        agent_node(i).plot(1) = plot(agent_node(i).xc,agent_node(i).yc,'mo','MarkerFaceColor','cyan','MarkerSize',10);
