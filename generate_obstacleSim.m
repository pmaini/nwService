function obstacle  = generate_obstacleSim(base,filename)
%%Generate obstacles which are non overlapping with each other and the base
%%station, within the world boundary. Record total
%%obstructed space. No neighbour field, since not needed.

global numObstacle xmax xmin ymax ymin Total_obstacle_space;
global numCells covered_fraction env_graph;

obstacle = struct([]);

if nargin == 2
    load(filename,'obstacle');
    for i = 1:numObstacle
